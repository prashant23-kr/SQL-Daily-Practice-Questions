...
WHERE salary_rank = 2;
 ...
 -- 06Display the total sales amount generated by each city along with the city name.

select c.city, sum(s.SalesAmount) from CustomerData c
inner join SalesData s
on c.CustomerID = s.CustomerID
group by c.city;

-- 07 Display the membership types whose total purchase value is greater than ₹10,00,000.


select c.membership, sum(s.SalesAmount) as Total_purchase
from customerData c
inner join SalesData s
on c.customerrID = s.customerID
group by c.membership
having having(s.salesAmount) > 1000000

-- 08 Rank customers based on their total purchase value within each city and display the top 2 customers from every city.


with Customer_Total as (
select c.customerID, c.customerName, c.city, sum(s.SalesAmount) as total_purchase
from CustomerData c
inner join SalesData s
on c.customerID = s.CustomerID
group by c.customerID, c.CusstomerName, c.city
),
ranked_customer as (
select customerID, customerName, city, total_purchase
rank() over(partition by city order by total_purchase) as rnk
),
select c.customerID, c.customeName, city, total_purchase
from ranked_customer
where rnk = 2;


04 Find the highest-selling product in each category based on total sales amount.


with Highest_Selling as (
select p.ProductID, p.ProductName, p.category, sum(s.salesAmount) as Total_sales
from ProductData p
inner join SalesData s
...
RankedProduct as (
select ProductID, ProductName, category, Total_sales,
rank () over(partition by category order by Total_sales) as rnk
from 
),
select ProductID, ProductName, city, total_Purchase
from rankedProduct
where rnk = 1;
